...
@interface FMDatabase : NSObject  {
    ...
    void*               _db;                     // 数据库链接handler
    NSString*           _databasePath;           // 数据库文件路径
    BOOL                _logsErrors;             // 是否打印error
    BOOL                _crashOnErrors;          // 错误出现，是否应该abort
    BOOL                _traceExecution;         // 是否应该打印执行的步骤
    BOOL                _checkedOut;
    BOOL                _shouldCacheStatements;  // 是否应该缓存stmt语句
    BOOL                _isExecutingStatement;   // sql语句是否正在执行，多线程操作DB的判断
    BOOL                _inTransaction;
    NSTimeInterval      _maxBusyRetryTimeInterval; // 数据库繁忙的超时间隔
    NSTimeInterval      _startBusyRetryTime;       // 记录第一次数据库繁忙开始重试的时间
    
    NSMutableDictionary *_cachedStatements;        // stmt的缓存容器[query:NSSet[stmt]]
    NSMutableSet        *_openResultSets;          // 结果集的容器
    NSMutableSet        *_openFunctions;           //
 
    NSDateFormatter     *_dateFormat;              // 时间格式，存放的时候是以时间格式还是时间戳的形式
}
