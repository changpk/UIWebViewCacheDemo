...
- (instancetype)initWithPath:(NSString*)aPath {
    /*
     sqlite支持三种模式0：单线程模式；1：串行模式；2：多线程模式，模式的指定可以在编译，启动，运行三个阶段指定
     编译：SQLITE_THREADSAFE这个宏，可以指定0：单线程模式；1：串行模式；2：多线程模式
     启动：编译时没有指定SQLITE_THREADSAFE为单线程模式，则启动的时候可以通过sqlite3_config()配置模式SQLITE_CONFIG_SINGLETHREAD，SQLITE_CONFIG_SERIALIZED，SQLITE_CONFIG_MULTITHREAD
     运行：如果没有在编译时和启动时指定为单线程模式，那么每个数据库连接在创建时可单独的被指定为多线程模式或者串行模式，但是不能指定为单线程模式。如果在编译时或启动时指定为单线程模式，就无法在创建连接时指定多线程或者串行模式。
     sqlite3_open_v2()函数的第三个参数来指定线程模式SQLITE_OPEN_NOMUTEX标识创建多线程模式的连接；SQLITE_OPEN_FULLMUTEX标识创建串行模式的连接
     作用域依次向下
     */
    // 检测是sqlite在编译期间的SQLITE_THREADSAFE预处理是否是多线程安全访问
...
- (const char*)sqlitePath {
    ...
    // 2.路径存在为空字符串，则创建到临时目录中
    if ([_databasePath length] == 0) {
        return ""; // this creates a temporary database (it's an sqlite thing).
    }
    
    // 3.返回指定路径（以文件格式const char *）
...
- (BOOL)close {
    ...
    // 关闭数据库之前，检测是否仍有数据库语句在执行，有的话则释放stmt语句再次执行关闭
    do {
        retry   = NO;
...
// 缓存的为什么用set来作为容器不直接缓存FMStatement对象
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    // 创建一个对象_openResultSets不会对对象进行strong reference
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;   // 返回结果
    sqlite3_stmt *pStmt      = 0x00;   // 执行的stmt
    FMStatement *cachedStmt  = 0x00;   // 封装的cachedStmt
    ...
    // 有缓存使用缓存，使用之前需要reset状态（应为stmt有个bind的过程）
    if (_shouldCacheStatements) {
    ...
    // stmt不存在，则需要创建stmt
    if (!pStmt) {
        // 把sql的text转换为执行的stmt类型，最后需要finalize销毁以防止内存泄露
        rc = sqlite3_prepare_v2(_db, [sql UTF8String], -1, &pStmt, 0);
